...
# # Convert accounts data to DataFrames
...
# # Define number of mutual funds per bank
# NUM_FUNDS_PER_BANK = 20
...
# # Function to generate Mutual Funds
# def generate_mutual_funds(bank_name, num_funds):
#     """
#     Generate a list of mutual funds for a given bank.
#     """
#     funds = []
#     for _ in range(num_funds):
#         fund_name = fake.company() + " Mutual Fund"
#         fund_type = random.choice(['Equity', 'Debt', 'Hybrid', 'Index', 'ELSS'])
...
#     """
#     clients = []
#     selected_indices = random.sample(range(NUM_UNIQUE_PERSONS), num_clients)
#     for idx in selected_indices:
#         person = unique_persons[idx]
#         # Depending on the bank, map to respective mutual funds client table
#         if bank_name == 'SBI':
#             client = {
#                 'FullName': random.choice(person['NameVariations']),
...
#         elif bank_name == 'Tata':
#             client = {
#                 'InvestorName': random.choice(person['NameVariations']),
...
# # Function to generate Investment Returns
# def generate_investment_returns(bank_name, clients, funds):
#     """
#     Generate investment returns for a given mutual funds bank.
#     """
#     returns = []
#     for client_id, client in enumerate(clients, 1):
#         num_investments = random.randint(1, 10)  # Number of investments per client
...
#                 units_purchased = round(random.uniform(1.0, 1000.0), 4)
...
#             elif bank_name == 'Tata':
#                 investment_date = fake.date_between(start_date=fund['LaunchDate'], end_date='today')
#                 units_bought = round(random.uniform(1.0, 1000.0), 4)
#                 investment_inr = round(units_bought * fund['NAV'], 2)
...
#                 investment = {
#                     'Tata_FundID': funds.index(fund) + 1,
...
#                 investment_date = fake.date_between(start_date=fund['LaunchDate'], end_date='today')
#                 units_purchased = round(random.uniform(1.0, 1000.0), 4)
#                 amount_invested = round(units_purchased * fund['NAV'], 2)
...
#                 investment = {
#                     'Bajaj_FundID': funds.index(fund) + 1,
...
# # Print Mutual Funds Investment Details for Verification
# print("\n--- Mutual Funds Investment Details ---\n")
...
#         client_pan = client['PAN_Number'] if bank_name == 'SBI' else (client['PAN'] if bank_name == 'Tata' else client['PAN_Num'])
...
#             print(f"Client: {client_name} (PAN: {client_pan}) has {num_investments} investments:")
...
# # Print SBI Mutual Funds Investment Details
...
# # Print Tata Mutual Funds Investment Details
# print_investment_details(
#     bank_name='Tata',
...
# # Print Bajaj Mutual Funds Investment Details
# print_investment_details(
#     bank_name='Bajaj',
...
def generate_mutual_funds(bank_name, num_funds):
    ...
    for _ in range(num_funds):
        if bank_name == 'SBI':
            ...
            fund_name = fake.company() + " Mutual Fund"
            category = random.choice(['Equity', 'Debt', 'Hybrid', 'Index', 'ELSS'])
            ...
            fund_title = fake.company() + " Mutual Fund"
            fund_category = random.choice(['Equity', 'Debt', 'Hybrid', 'Index', 'ELSS'])
...
def generate_mf_clients(bank_name, num_clients):
    ...
    for idx, person in enumerate(selected_persons):
        # Depending on the bank, map to respective mutual funds client table
        # Use the same name variation across all mutual funds in the same company
        name_variation = random.choice(person['NameVariations'])
        if bank_name == 'SBI':
            client = {
                'SBI_ClientID': idx + 1,
...
def generate_investment_returns(bank_name, clients, funds):
    ...
    for client in clients:
        ...
        num_investments = random.randint(1, 5)  # Number of investments per client
        for _ in range(num_investments):
            fund = random.choice(funds)
            if bank_name == 'SBI':
                investment_date = fake.date_between(start_date=fund['LaunchDate'], end_date='today')
