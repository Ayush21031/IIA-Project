...
def generate_full_name():
    """
    Generates a full name along with first, middle, and last names.
    """
...
def generate_name_variations(first_name, middle_name, last_name):
    """
    Generates different variations of a full name.
...
def generate_stock_symbol(existing_symbols):
    """
    Generates a unique 4-letter stock symbol.
    """
    while True:
...
def generate_stocks(platform, num_stocks):
    """
    Generates a list of unique stocks for a given platform.
    """
    stocks = []
...
def generate_stock_clients(platform, platform_clients_list, person_investments, client_id_col, name_col, email_col, phone_col, pan_col, address_col, dob_col, account_date_col):
    ...
    for investment in person_investments:
        ...
        if platform in investment['StockPlatforms']:
            ...
            if person:
                client = {
                    name_col: name_variation,
                    email_col: fake.unique.email(),
                    phone_col: fake.phone_number(),
                    pan_col: pan,
                    address_col: person['Address'],
                    dob_col: person['DateOfBirth'],
                    account_date_col: fake.date_between(start_date=person['AccountCreationDate'], end_date='today')
                }
                platform_clients_list.append(client)
    return pd.DataFrame(platform_clients_list)
...
def generate_stock_orders(platform, clients_df, stocks_df, orders_list, client_id_col, stock_id_col, order_type_col, quantity_col, price_col, total_col, order_date_col, status_col):
    ...
    for _, client in clients_df.iterrows():
        ...
        for _ in range(num_orders):
            ...
            else:  # Default for 'ICICI', 'HDFC', 'SBI' platforms if any
                order = {
                    'ClientID': client[client_id_col],
                    'StockID': stock[stock_id_col],
...
def generate_stock_prices(platform, stocks_df, prices_list, stock_id_col):
    """
...
# -------------------- Introduce Distinct Individuals with Similar Names --------------------
...
for person in additional_persons:
    ...
    # Assign unique name variations if possible
    for bank_idx, bank in enumerate(banks):
        # Assign a unique name variation to each bank
        variations = person['NameVariations']
        name = variations[bank_idx % len(variations)]
...
# -------------------- Stock Platforms Data Generation --------------------
...
# Function to generate clients for stock platforms is already defined above
...
# Function to generate orders for stock platforms is already defined above
...
# Function to generate stock prices is already defined above
...
# -------------------- Mutual Funds Clients and Investment Returns Generation --------------------
...
# -------------------- Convert Mutual Funds Clients and Investments to DataFrames --------------------
...
# -------------------- Save Stock Platform Data to CSV Files --------------------
...
# -------------------- Save Bank Accounts Data to CSV Files --------------------
...
# Define transaction types for each bank
icici_transaction_types = ['Deposit', 'Withdrawal', 'Transfer', 'Payment']
hdfc_transaction_types = ['Deposit', 'Withdrawal', 'Transfer', 'Payment']
sbi_transaction_types = ['Deposit', 'Withdrawal', 'Transfer', 'Payment']

# Initialize lists to hold transactions data
icici_transactions_data = []
hdfc_transactions_data = []
sbi_transactions_data = []

def generate_transactions(account_id, account_creation_date, transaction_types, bank_name):
    """
    Generate a list of transactions for a given account.
    """
    transactions = []
    num_transactions = random.randint(10, 100)  # Number of transactions per account

    for _ in range(num_transactions):
        transaction_id = None  # To be assigned later based on list length
        transaction_date = fake.date_between(start_date=account_creation_date, end_date='today')
        transaction_type = random.choice(transaction_types)
        
        # Define amount ranges based on transaction type
...
# Generate transactions for ICICI accounts
print("Generating ICICI Transactions...")
for idx, account in icici_df.iterrows():
    transactions = generate_transactions(
        account_id=account['ICICI_AccountID'],
        account_creation_date=account['LastUpdated'],
        transaction_types=icici_transaction_types,
        bank_name='ICICI'
    )
    for txn in transactions:
        txn_record = {
            'TransactionID': len(icici_transactions_data) + 1,
            'ICICI_AccountID': account['ICICI_AccountID'],
            'TransactionDate': txn['TransactionDate'],
            'TransactionType': txn['TransactionType'],
            'Amount': txn['Amount'],
            'Description': txn['Description']
        }
        icici_transactions_data.append(txn_record)

# Generate transactions for HDFC accounts
print("Generating HDFC Transactions...")
for idx, account in hdfc_df.iterrows():
    transactions = generate_transactions(
        account_id=account['HDFC_AccountID'],
        account_creation_date=account['LastUpdate'],
        transaction_types=hdfc_transaction_types,
        bank_name='HDFC'
    )
    for txn in transactions:
        txn_record = {
            'TransactionID': len(hdfc_transactions_data) + 1,
            'HDFC_AccountID': account['HDFC_AccountID'],
            'TransDate': txn['TransactionDate'],
            'Type': txn['TransactionType'],
            'TransAmount': txn['Amount'],
            'Notes': txn['Description']
        }
        hdfc_transactions_data.append(txn_record)

# Generate transactions for SBI accounts
print("Generating SBI Transactions...")
for idx, account in sbi_df.iterrows():
    transactions = generate_transactions(
        account_id=account['SBI_AccountID'],
        account_creation_date=account['Last_Updated'],
        transaction_types=sbi_transaction_types,
        bank_name='SBI'
    )
    for txn in transactions:
        txn_record = {
            'TransactionID': len(sbi_transactions_data) + 1,
            'SBI_AccountID': account['SBI_AccountID'],
            'Date': txn['TransactionDate'],
            'Type_of_Transaction': txn['TransactionType'],
            'Amount': txn['Amount'],
            'Details': txn['Description']
        }
        sbi_transactions_data.append(txn_record)

# Convert transactions data to DataFrames
icici_txn_df = pd.DataFrame(icici_transactions_data)
hdfc_txn_df = pd.DataFrame(hdfc_transactions_data)
sbi_txn_df = pd.DataFrame(sbi_transactions_data)

# -------------------- Save Transactions Data to CSV Files --------------------

icici_txn_df.to_csv('ICICI_Transactions.csv', index=False)
hdfc_txn_df.to_csv('HDFC_Transactions.csv', index=False)
sbi_txn_df.to_csv('SBI_Transactions.csv', index=False)
...
print("ICICI_Transactions Sample Data:")
print(icici_txn_df.head(), "\n")

print("HDFC_Transactions Sample Data:")
print(hdfc_txn_df.head(), "\n")

print("SBI_Transactions Sample Data:")
print(sbi_txn_df.head(), "\n")

# -------------------- Verification: Print Detailed Investment Details --------------------
...
print("Synthetic data generation for banks, mutual funds, and stock platforms completed and saved to CSV files.")
